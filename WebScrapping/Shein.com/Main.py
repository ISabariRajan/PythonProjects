...
def capture_page_as_soup(driver, url):
    print(url)
    driver.get(url)
    selenium.WebDriverWait(driver, 10)
    soup = selenium.convert_webpage_to_soup(driver)
    pages.append(soup)
    # pool.submit(get_items_from_page, (soup))

def pick_item_info(driver):
    url = base_url + sub_path
    capture_page_as_soup(driver, url)
    pages_count = selenium.driver.find_elements(selenium.By.CLASS_NAME, "sui-pagination__total")[0].text
...
selenium.run_function_within_webdriver(pick_item_info)